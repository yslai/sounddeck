...
protected:
	...
	uint32_t alsa_write(void *buffer, uint32_t sample_frame_count)
	{
		...
		unsigned char *o = new unsigned char[channel_step * sample_frame_count];
...
public:
	...
	HRESULT	QueryInterface(REFIID id, void **outputInterface)
	{
		...
		if (memcmp(&id, &IID_IDeckLinkOutput, size_iid) == 0) {
			if (_forward != NULL) {
				...
				HRESULT result = _forward->QueryInterface
					(id, reinterpret_cast<void **>(&output));
				*outputInterface = new SoundDeckLinkOutput(output);
				return result;
			}
			else {
	...
	SoundDeckLink(IDeckLink *forward = NULL)
		: _forward(forward)
	{
	}
	SoundDeckLink(std::pair<std::string, std::string> alsa_device)
		: _forward(NULL), _alsa_device(alsa_device.first),
...
protected:
	...
	std::vector<std::pair<std::string, std::string> >::const_iterator _iterator_alsa_device;
	size_t _count;
